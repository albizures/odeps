...
test_get_contents_url :: proc(t: ^testing.T) {
    ...
    // Case 1: Base repo
    {
        ...
        actual, ok := odeps.get_contents_url(url, commit)
    ...
    // Case 3: Invalid URL
    {
        ...
        commit := "123456"
        _, ok := odeps.get_contents_url(url, commit)
...
test_is_folder :: proc(t: ^testing.T) {
    ...
    testing.expect(t, odeps.is_folder("https://github.com/odin-lang/Odin"), "Should be a folder")

    // Case 2: Valid subfolder
    testing.expect(t, odeps.is_folder("https://github.com/odin-lang/Odin/tree/master/core"), "Should be a folder")

    // Case 3: Non-GitHub URL
    testing.expect(t, !odeps.is_folder("https://google.com/odin-lang/Odin"), "Should not be a GitHub folder")

    // Case 4: Invalid GitHub URL (too short)
    testing.expect(t, !odeps.is_folder("https://github.com/odin-lang"), "Should not be a folder (missing repo)")
}
...
test_convert_link_to_api :: proc(t: ^testing.T) {
    ...
    // Case 1: Base repo
    {
        ...
        actual, ok := odeps.convert_link_to_api(url)
